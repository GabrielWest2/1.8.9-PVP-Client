...
import customclient.mods.AutoGGMod;
import customclient.mods.HypixelChatFilterMod;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    {
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);
        IChatComponent c = packetIn.getChatComponent();
        if (packetIn.getType() == 2)
        {
            this.gameController.ingameGUI.setRecordPlaying(c, false);
        }
        else
        {
        	HypixelChatFilterMod m = Main.INSTANCE.getMod(HypixelChatFilterMod.class);
        	if(m!= null && m.isEnabled()) {
	        	String message = HypixelChatFilterMod.removeColorCodes(c.getUnformattedText());
	        	System.out.println(message);
	        	
	        	boolean showMessage = true;
	        	
	        	if(HypixelChatFilterMod.hasAny(message, HypixelChatFilterMod.partyTexts)) {
	        		showMessage = m.showPartyMessages;
	        		
	        	}else if(HypixelChatFilterMod.hasAny(message, HypixelChatFilterMod.friendTexts)) {
	        		showMessage = m.showFriendMessages;
	        		
	        	}else if(HypixelChatFilterMod.hasAny(message, HypixelChatFilterMod.teamTexts)) {
	        		showMessage = m.showTeamMessages;
	        		
	        	}else if(HypixelChatFilterMod.hasAny(message, HypixelChatFilterMod.joinTexts)) {
	        		showMessage = m.showJoinMessages;
	        		
	        	}else if(HypixelChatFilterMod.hasAny(message, HypixelChatFilterMod.leaveTexts)) {
	        		showMessage = m.showQuitMessages;
	        	}else if(message.startsWith("[")) {
	        		showMessage = m.showChatMessages;
	        	}else {
	        		showMessage = m.showOtherMessages;
	        	}
	        	if(showMessage)
	        		this.gameController.ingameGUI.getChatGUI().printChatMessage(c);
        	}else {
        		this.gameController.ingameGUI.getChatGUI().printChatMessage(c);
        	}
        }
    ...
    public void handleTitle(S45PacketTitle packetIn)
    {
        ...
        AutoGGMod mod = Main.INSTANCE.getMod(AutoGGMod.class);
        if(mod != null && mod.isEnabled()) {
        	boolean winMsg = s2.contains("VICTORY!");
        	boolean lossMsg = s2.contains("GAME OVER!") || s2.contains("DEFEAT");
        	if((winMsg && mod.ggOnWin) || (lossMsg && mod.ggOnLoss)) {
