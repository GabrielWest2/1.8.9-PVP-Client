...
import customclient.mods.BossBarMod;
import customclient.mods.ScoreboardMod;
...
public class GuiIngame extends Gui
{
	...
	public void renderScoreboard(boolean demo)
    {
		
		
		ScaledResolution sr = new ScaledResolution(mc);
		 Scoreboard scoreboard = this.mc.theWorld.getScoreboard();
		ScoreObjective scoreobjective1 = scoreboard.getObjectiveInDisplaySlot(1);


		ScoreboardMod mod = Main.INSTANCE.getMod(ScoreboardMod.class);
		if(demo) {
			if (mod == null || mod.enabled == false)
	            return;
		}else {
			if (scoreobjective1 == null || mod == null || mod.enabled == false)
				return;			
		}
        ...
        if(!demo) {
	        ...
	        int width = this.getFontRenderer().getStringWidth(scoreobjective1.getDisplayName());
	
	        for (Score score : collection)
	        {
	            ScorePlayerTeam scoreplayerteam = scoreboard.getPlayersTeam(score.getPlayerName());
	            String s = mod.showNums ? ScorePlayerTeam.formatPlayerName(scoreplayerteam, score.getPlayerName()) + ": " + EnumChatFormatting.RED + score.getScorePoints():
	        ...
	        int height = (collection.size()+1) * this.getFontRenderer().FONT_HEIGHT;
	        int y = height +1 + mod.position.calculateY();
	        int x = mod.position.calculateX()+mod.hudWidth-width;
	        int j = 0;
	        ...
	        for (Score score1 : collection)
	        {
	            ...
	            int k = y - j * this.getFontRenderer().FONT_HEIGHT;
	            int l = width +x;
	            
	            drawRect(x - 2, k, l, k + this.getFontRenderer().FONT_HEIGHT, 1342177280);
	            this.getFontRenderer().drawString(s1, x, k, 553648127);
	            if(mod.showNums)
	            ...
	            if (j == collection.size())
	            {
	                ...
	                drawRect(x - 2, k - this.getFontRenderer().FONT_HEIGHT - 1, l, k - 1, 1610612736);
	                drawRect(x - 2, k - 1, l, k, 1342177280);
	                this.getFontRenderer().drawString(s3, x + width / 2 - this.getFontRenderer().getStringWidth(s3) / 2, k - this.getFontRenderer().FONT_HEIGHT, 553648127);
	            }
	        }
        }else {
        	int width = this.getFontRenderer().getStringWidth("§e§lBED WARS");
        	List<String> lines = Arrays.asList("§ewww.hypixel.net", "", "Total Wins: §a100", "Total Kills: §a1,000", "", "Coins: §6100,000","","Loot Chests: §e10", "", " §8[§b■■■■■■§7■■■■§8]", "Progress: §b5.0k§7/§a8k", "", "Level: §e99✫", "", "§702/26/23  §8L22H");
        	int i = 1;
            for (String score : lines)
            {
                ScorePlayerTeam scoreplayerteam = scoreboard.getPlayersTeam(score);
                String s =ScorePlayerTeam.formatPlayerName(scoreplayerteam, score)+ ": " + EnumChatFormatting.RED + i++;
                width = Math.max(width, this.getFontRenderer().getStringWidth(s));
            }
            width-=2;
            int height = (lines.size()+1) * this.getFontRenderer().FONT_HEIGHT;
            int y = height +1 + mod.position.calculateY();
            int x = mod.position.calculateX()+mod.hudWidth-width-2;
            int j = 0;
            i = 1;
            for (String score1 : lines)
            {
                ++j;
                ScorePlayerTeam scoreplayerteam1 = scoreboard.getPlayersTeam(score1);
                String s1 = ScorePlayerTeam.formatPlayerName(scoreplayerteam1, score1);
                String s2 = EnumChatFormatting.RED + "" + i++;
                int k = y - j * this.getFontRenderer().FONT_HEIGHT;
                int l = width +x  + 2;
                ...
                if (j == lines.size())
                {
                    String s3 = "§e§lBED WARS";
                    drawRect(x - 2, k - this.getFontRenderer().FONT_HEIGHT - 1, l, k - 1, 1610612736);
                    drawRect(x - 2, k - 1, l, k, 1342177280);
                    this.getFontRenderer().drawString(s3, x + width / 2 - 26, k - this.getFontRenderer().FONT_HEIGHT, 553648127);
                }
            }
        }
    }
     ...
     */
    public void renderBossHealth(boolean dummy)
    {
    	GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        this.mc.getTextureManager().bindTexture(icons);
        GlStateManager.enableBlend();
    	GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
    	boolean isBossBarModEnabled = Main.INSTANCE.getMod(BossBarMod.class).isEnabled();
    	if(!dummy) {
	        if (BossStatus.bossName != null && BossStatus.statusBarTime > 0 && isBossBarModEnabled)
	        {
	            ...
	            j = Main.INSTANCE.getMod(BossBarMod.class).position.calculateX();
	            b0 = Main.INSTANCE.getMod(BossBarMod.class).position.calculateY()+11;
	            ...
	            this.getFontRenderer().drawStringWithShadow(s, (float)(j + short1/2 - this.getFontRenderer().getStringWidth(s) / 2), (float)(b0 - 10), l);
	            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
	            this.mc.getTextureManager().bindTexture(icons);
	        }
    	}else {
