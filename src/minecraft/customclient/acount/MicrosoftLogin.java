...
public class MicrosoftLogin {
    ...
    private static final String CLIENT_ID = "23ac87d1-0f10-4900-bdf2-d17a162d0ecf";
    ...
    public static void getRefreshToken(Consumer<String> callback) {
        ...
        try {
			Desktop.getDesktop().browse(new URI("https://login.live.com/oauth20_authorize.srf?client_id=" + CLIENT_ID + "&response_type=code&redirect_uri=http://127.0.0.1:" + PORT + "&scope=XboxLive.signin%20offline_access"));
		} catch (IOException e) {
    ...
    public static void startServer() {
        ...
        try {
        	System.out.println("Opening on " + new InetSocketAddress("127.0.0.1", PORT).toString());
            server = HttpServer.create(new InetSocketAddress("127.0.0.1", PORT), 0);

            server.createContext("/", new Handler());
            server.setExecutor(LoginExecutor.executor);
            server.start();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static class Handler implements HttpHandler {
        ...
        private void handleCode(String code) {
            AuthTokenResponse res = Http.post("https://login.live.com/oauth20_token.srf")
                ...
                .sendJson(AuthTokenResponse.class);
        	System.out.println(res.refresh_token);
